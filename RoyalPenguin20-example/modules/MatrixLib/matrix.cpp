...
#define check_size(mat) if (!((mat).m_ == m_ && (mat).n_ == n_)) printString("ERROR: class matrix exception: bad size exception\n");
#define for_ij(m, n) for (uint i = 0; i < (m); i++) for (uint j = 0; j < (n); j++)

matrix::matrix(uint rows, uint cols)
		: m_{rows}, n_{cols}, buf_{(m_==0||n_==0) ? nullptr : (double*)malloc(sizeof(double)*m_*n_)} {
...
matrix::matrix(uint rows, uint cols, std::initializer_list<double> list)
		: m_{rows}, n_{cols}, buf_{(m_==0||n_==0) ? nullptr : (double*)malloc(sizeof(double)*m_*n_)} {
	if (list.size() != (m_*n_))
		printString("ERROR: class matrix exception: List must be of same size as matrix\n");
...
matrix::matrix(const matrix& mat)
		: m_{mat.m_}, n_{mat.n_}, buf_{(m_==0||n_==0) ? nullptr : (double*)malloc(sizeof(double)*m_*n_)} {
...
double matrix::get(uint i, uint j) const {
	if (i >= m_ || j >= n_)
		printString("ERROR: class matrix exception: Term isn't within matrix\n");
...
void matrix::set_row(uint i, const matrix& row) {
	if (row.m_ != 1 || row.n_ != n_)
		printString("ERROR: class matrix exception: bad size exception\n");
	for (int j = 0; j < n_; j++)
		(*this)(i, j) = row(0, j);
}
...
double matrix::trace() const {
	if (!is_square())
		printString("ERROR: class matrix exception: not square exception\n");
	double trace = 0;
	for (int i = 0; i < m_; i++)
		trace += (*this)(i, i);
	return trace;
...
matrix matrix::invert() const {
	...
	double deter = det();
	if (deter == 0)
		printString("ERROR: class matrix exception: not invertible exception\n");
	matrix mat;
	if (m_ == 2) {
		mat = matrix(2, 2);
		mat(0, 0) = (*this)(1, 1);
...
matrix matrix::solve(const matrix& ans) const {
	...
	double deter = det();
	if (deter == 0)
		printString("ERROR: class matrix exception: not solvable exception\n");
	matrix res(m_, 1);
	matrix mat;
	for (uint j = 0; j < n_; j++) {
		mat = *this;
