...
#include "../DebugLib/print_lib.hpp"
	...
	simple_matrix::matrix parse_matrix(InputIterator& start, const InputIterator& end) {
		...
		while (!(i == end || cancel)) {
			...
			case VOID:
				...
				default:
					// Unexpected value
					printString("ERROR: class matrix exception: Unexpected character encountered\n");
			...
			case CAPTURE:
				...
				case ']':
					...
					m++;
					if (!(n == 0 || cn == n)) {
						printString("ERROR: class matrix exception: Column lengths must be consistent\n");
		...
		if (STATE == CAPTURE) {
			...
			printString("ERROR: class matrix exception: Premature end\n");
		}
		matrix M{m, n};
		for (uint i = 0; i < m; i++)
		for (uint j = 0; j < n; j++) {
